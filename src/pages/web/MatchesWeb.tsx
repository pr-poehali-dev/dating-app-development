...
export default function MatchesWeb() {
  ...
  const loadMatches = () => {
    ...
    const matchedUsers: User[] = savedMatches.map((matchId: string, index: number) => ({
      id: matchId,
      name: ['Анна', 'Максим', 'София', 'Елена', 'Дмитрий', 'Мария', 'Александр', 'Виктория'][index] || `Пользователь ${index + 1}`,
      email: `user${index + 1}@example.com`,
      age: 22 + Math.floor(Math.random() * 15),
      bio: [
        'Люблю путешествия и новые знакомства ✈️ В поисках интересных разговоров и приключений',
        'Увлекаюсь спортом и активным отдыхом 🏃‍♂️ Всегда готов к новым вызовам и открытиям',
        'Художница в душе, ищу вдохновение 🎨 Творчество — моя страсть и способ самовыражения',
        'Книголюб и любитель кофе ☕️ Обожаю уютные вечера с хорошей книгой и интересными беседами',
        'Программист днём, музыкант ночью 🎵 Создаю код и мелодии с одинаковой страстью',
        'Фотограф и путешественница 📸 Ловлю моменты и собираю воспоминания по всему миру',
        'Спортсмен и любитель природы 🏔️ Покоряю вершины и исследую новые маршруты',
        'Дизайнер и мечтательница ✨ Создаю красоту и верю в силу хорошего дизайна'
      ][index] || 'Привет! Давай знакомиться!',
      photos: [],
      location: {
        lat: 55.7558,
        lng: 37.6176,
        city: ['Москва', 'Санкт-Петербург', 'Екатеринбург', 'Новосибирск', 'Казань'][index % 5]
      },
      interests: [
        ...
        ['Фотография', 'Путешествия', 'Природа'],
        ['Спорт', 'Горы', 'Альпинизм'],
